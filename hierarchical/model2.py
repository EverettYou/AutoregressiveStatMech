...
class GraphConv(nn.Module):
    ...
    def forward(self, input, j = None):
        ...
        graph = self.graph
        if j is not None: # forward from specific node
            mask = (graph[0] == j) # create a mask for edges
            graph = graph[:, mask]
        if not self.self_loop: # no self loop
            mask = (graph[2] != 0) # create a mask for edges
            graph = graph[:, mask]
        return self.propagate(graph, input)

    def propagate(self, graph, input):
